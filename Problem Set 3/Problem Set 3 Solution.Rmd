...
The feature space is expanded via a polynomial transformation up to degree 3.
...
$Lap Distance$ is retained in the design matrix; while its linear influence was eliminated via residualization, its inclusion allows the Lasso algorithm to probe for potential high-order interactions with other track conditions.
...
The top three predictors dominate the model, accounting for over $97\%$ of the total feature importance.
Positive coefficients for $Differential$ and $Brake Balance$ indicate that increasing these parameters correlates with slower adjusted lap times.
Furthermore, the significant $Engine \times Lap Distance$ interaction suggests that the sensitivity of performance to engine settings scales with track length.
It is crucial to note that ranking importance solely by coefficient magnitude has limitations; polynomial terms with smaller coefficients may still exert significant influence due to variable scaling.
...
The optimization yielded boundary solutions for five of the six parameters, selecting either the range minimum or maximum.
$Suspension$ was the sole feature to exhibit an interior optimum, indicating a convex trade-off.
Paradoxically, although $Brake Balance$, $Differential$, and $Engine$ initially displayed positive coefficients (implying a detrimental effect on speed), the algorithm maximized these settings.
This suggests that complex interaction effects and higher-order polynomial terms dominate the linear penalties at the upper bounds, effectively inverting the relationship to minimize lap time.
