...
[
\lambda = 0.162886,\quad \text{df}=6,\quad \text{AICc}=2.338800,\quad R^2=0.152070.
]
...
Applying the AICc-selected Lasso model (segment **`seg18`**, (\lambda \approx 0.163)) to the unrated 101st song yields a predicted rating of
[
\hat{y} = 2.498992 \approx 2.50 \text{ stars}.
]
Since ratings are defined on a 1–5 scale, we optionally clip predictions to the valid range ([1,5]); here, the prediction already lies within this interval and remains unchanged.
...
# Optional (nur wenn Sie den Pfad wirklich sehen wollen):
plot(fit_path)
...
As expected, (\lambda_{1se}) is larger (stronger regularization), leading to a substantially more parsimonious model.
...
predict_lap_time_wrapper <- function(x) {
  ...
  prediction <- predict(final_ols, newdata = X_df)
