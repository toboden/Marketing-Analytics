...
# --> Add absolute paths in line 38 (to venv), 50 (where your song files are),

# The feature extraction is commented out for better performance -> uncomment for replication!
##############################
library(reticulate)
...
cat("Gerundet auf Sterne-Bewertung:", round(value))
...
We continiue, by screening the dataset for pronounced correlations between variables.
...
Correlations between conditions and $Lap Time$ are negligible, with the exception of a dominant relationship with $Lap Distance$.
This is axiomatic, as $Lap Time = \frac{Lap Distance}{Average Velocity}$.
...
Re-assessing correlations with $Adjusted Lap Time$ reveals discernible dependencies previously masked by track length.
...
A quadratic regression yields a superior fit (higher $R^2$) compared to a logarithmic transformation, better capturing the non-linear structure of the data distribution.
...
This paradox implies the presence of significant interaction effects, necessitating further multivariate investigation.
...
We conclude the visual inspection here, as further manual exploration yields diminishing returns.
Consequently, we rely on the Lasso algorithm to systematically identify the remaining relevant interactions.
...
The final model retains 23 predictors, explaining $26.7\%$ of the variance ($R^2 = 0.267$).
...
An analysis of the standardized feature importance reveals that the predictive model is primarily driven by vehicle configuration parameters rather than isolated track conditions.
The top ten most influential features consist exclusively of car parameters or their interactions with track variables, indicating that internal vehicle calibration supersedes static environmental factors in determining performance variability.

The $Differential$ exhibits the highest standardized impact with a positive linear coefficient, initially suggesting a baseline penalty for higher differential locking values.
Crucially, however, this effect is non-linear: multiple interaction terms involving the quadratic differential (e.g., $Brake.Balance \times `Differential^2$ and $Front.Wing \times Differential^2$) carry negative coefficients.
This counter-effect implies a concave optimization surface, where the theoretical penalty of a high differential is mitigated or reversed when strictly coordinated with specific brake and aerodynamic settings.

Furthermore, the $Engine$ parameter demonstrates distinct context-dependency.
The interaction $Engine \times Lap.Distance$ presents a positive coefficient, implying a performance detriment on longer tracks---potentially attributable to unobserved factors such as fuel weight or efficiency constraints in the simulation physics.
Conversely, the interaction $Inclines \times Engine^2$ is negative, confirming that higher engine output yields a net reduction in lap time specifically in high-load scenarios, such as steep ascents.
Finally, the prominence of the $Front.Wing \times Rear.Wing^2$ interaction (negative coefficient) underscores the critical importance of aerodynamic balance, indicating that downforce components must be tuned in concert rather than in isolation to maximize speed.
...
We employ the L-BFGS algorithm, a quasi-Newton method designed for unconstrained non-linear optimization tasks.
...
The optimization results indicate that the objective function is monotonic for most parameters within the valid range, leading to boundary solutions for five out of six variables. Specifically, $Rear Wing$, $Front Wing$, $Engin$, $Brake Balance$, and $Differential$ converged to their respective lower or upper constraints, implying that the optimal configuration lies at the edges of the permissible design space. Only $Suspension$ ($x \approx 104$) exhibited an interior optimum, suggesting a convex trade-off between stiffness and mechanical grip.

A notable finding is the maximization of $Differential$, and $Engine$ to their upper bounds (500). This appears counter-intuitive given their positive linear coefficients, which theoretically penalize lap time. However, this result confirms the dominance of the non-linear structure identified in the model: strong negative interaction effects and polynomial terms (e.g., $Brake.Balance \times Differential^2$) effectively invert the baseline penalty at high values, creating a net performance gain that outweighs the linear costs. Conversely, the minimization of aerodynamic components ($x=1$) suggests a strategy prioritizing drag reduction over downforce for the specific conditions of the target track.

The predicted value of $-1.9132$ represents the adjusted residual time, indicating that this optimized configuration is expected to complete the lap approximately 1.91 seconds faster than the baseline average predicted solely by the track's length.
