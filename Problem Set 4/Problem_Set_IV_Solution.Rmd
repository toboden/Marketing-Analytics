...
For the Italy race we started with the same “data-driven region search” concept as in Belgium, but replaced the query with the Italy/Monza track and weather values. As in Belgium, we first removed the lap-distance baseline by using a linear adjustment of lap time on lap distance, so that comparisons across simulator rows reflect setup differences rather than track length effects.

To identify a promising region in the 6D setup space (Front Wing, Rear Wing, Brake Balance, Suspension, Engine, Differential), we retrieved simulator rows most similar to Italy conditions via KNN on the fixed track+weather features. We implemented the neighbor selection deterministically (distance ordering with a stable row-id tie-breaker) to ensure reproducibility. The retrieved neighbor setups were then clustered (KMeans, and additionally hierarchical clustering as a robustness check) to detect coherent “strategies” in the neighborhood. We selected the cluster with the lowest mean adjusted lap time and derived conservative search bounds using the 5th and 95th percentiles (q05–q95) of each decision variable within that best-performing cluster. Inside those bounds we generated 30 candidate configurations using Latin Hypercube Sampling (LHS) and transferred them into the practice interface for evaluation.

After testing the first 30 model-suggested setups in practice, we deliberately adapted our decision-making based on observed performance. In particular, we applied the key empirical lesson from Belgium: the optimum often lies near the boundary of the allowed parameter range. Instead of continuing to sample uniformly inside the model-derived region, we performed a systematic boundary-focused local search around the best observed candidates, i.e., we explored extreme “edge” configurations in the direction suggested by the practice results. This deviation from the model proposal was highly effective: compared to the initial region suggested by clustering and LHS, the boundary-driven exploration improved lap times by roughly two seconds. Empirically, the fastest Italy setups concentrated near a boundary-solution pattern with very high Front/Rear Wing values combined with low values for the remaining knobs, especially Engine and Suspension; Brake Balance and Differential remained in a relatively stable low-to-mid range and were then fine-tuned around the best candidate.

We chose this two-stage approach because it balances exploration and exploitation under a tight practice-lap budget. KNN + clustering provides a cheap global prior from the simulator dataset, quickly narrowing the search to a plausible high-performance region without brute-forcing the full space. LHS then covers that region efficiently in the first practice round. Once practice data revealed a clear direction of improvement, boundary/local search became the most resource-efficient way to exploit the signal. In terms of adaptation, decisions were explicitly performance-driven: slow configurations were eliminated after the first round (e.g., lap times clearly above the competitive range), remaining attempts were allocated to replicate the best candidates for robustness and to probe boundary points that extend the best observed pattern. This led to an increasingly structured exploitation strategy instead of continued random sampling.

Regarding optimality, we are moderately confident that the final Italy setup is close to optimal within the explored neighborhood. The main evidence is the consistency of the top candidates across repeated evaluations (small dispersion), the observed convergence trend (moving toward extreme wing values and low Engine/Suspension produced monotonic improvements that eventually plateaued), and the fact that the best setup sits at or near the parameter bounds, which is a common signature of constrained optima in tuning problems. At the same time, we do not claim a formal global optimum: the search is limited by the practice budget and by the fact that only a subset of the full space can be evaluated. Our confidence therefore rests on convergence behavior and repeatability rather than an exhaustive proof.

## 2. Strategy (Italy)

Race strategy was optimized independently of setup, which turns the problem into a deterministic optimization task once the relevant parameters are identified. The key quantities were: a constant fuel burn per lap (independent of tyre compound), a fuel time penalty capturing that lap time increases linearly with fuel load (“less fuel → faster”), compound-specific base pace offsets because tyre degradation does not affect lap time but tyre choice does, a hard tyre-life feasibility constraint (tyres cannot be driven to 0% condition) enforced via given wear rates with a 10% safety buffer, and the pit stop loss of approximately 30 seconds with a maximum refuel level of 120 liters. With these ingredients, total race time can be expressed as the sum over all laps of (tyre base pace + fuel penalty depending on fuel remaining) plus pit stop losses at the end of planned laps.

We optimized the strategy by enumerating feasible plans under the submission-interface constraints (bounded number of planned stops), computing the total predicted race time for each candidate, and selecting the Top-3 strategies with minimal objective value. Feasibility was checked against the platform’s forced-pit logic: if fuel or tyre condition would not allow completing another lap, the car pits automatically and applies the next unused planned stop instruction; if none remain, the last planned stop is repeated as often as needed. This makes the optimization transparent and reproducible: given the estimated fuel penalty, tyre base paces, tyre-life limits with buffer, and pit loss, the best strategy follows directly from minimizing a deterministic objective under explicit constraints.

We chose this method because tyre degradation has no lap-time effect in this environment; strategy therefore reduces to choosing compound sequences and stint lengths under tyre-life and fuel constraints, while balancing pit loss against the benefit of lighter fuel loads. This structure is well-suited to an explicit optimizer rather than manual heuristics. Adaptation is straightforward: if additional practice evidence changes the estimated fuel sensitivity or tyre offsets, the optimizer is rerun with updated inputs and the recommended Top-3 strategies update automatically. Finally, we are highly confident in the optimizer’s output given the modeling assumptions, because the search is exhaustive within the defined constraints, all hard constraints (fuel capacity, tyre-life with buffer, pit loss, forced-pit rules) are enforced explicitly, and the resulting strategies are validated by simulation of the platform rule set. The remaining uncertainty primarily stems from estimation noise in the fuel penalty and tyre base pace offsets, not from the optimization procedure itself.
