...
In the first phase we estimate the main effects of the six setup variables, holding tyre compound and fuel strategy constant. We use D-optimality to come up with an experimental design, which maximizes the precision of the car setup coefficients while minimizing the number of experimental runs required to capture non-linear effects.
```{r, echo=FALSE}
library(AlgDesign)
...
```
The resulting design achieves a D-efficiency of 57.9% and maintains low correlations (maximum of 0.15) between the factors, ensuring that the influence of each individual parameter can be statistically isolated during analysis. Consequently, this approach maximizes the information gathered about the vehicle's performance while significantly minimizing the total number of practice laps required for testing.
