...
The plots indicate that K=4 is a promising candidate, so the following k-Means is fitted with 4 clusters.
...
The cluster summary, showing the mean of each car-feature parameter and the average adjusted lap time, identifies clusters 2 and 3 as the most promising candidates.
Cluster 3 exhibits a slightly superior lap time and a larger sample size, suggesting a more robust estimate.
Both clusters display very low values for Engine, Differential, and Suspension, while Brake Balance falls within the lower-to-middle range.
However, the wing parameters present a contradiction: while cluster 3 indicates high values, particularly for the Rear Wing, cluster 2 suggests mid-range values.
...
Visual inspection suggests a range of 4 to 5 clusters; ultimately, 4 is selected to maintain consistency with the K-Means analysis.
